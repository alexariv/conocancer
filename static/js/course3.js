...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const videoMap = {
    en: {
      1: "SCOA_Breast_Cancer__03E.mp4",
      2: "BRCA_Patient_03E.mp4",
      3: "How_to_self_exam_Get_Mammo_03E.mp4", // Corrected filename
    },
    es: {
      1: "SCOA_Breast_Cancer_03S.mp4",
      2: "BRCA_Patient_03S.mp4",
      3: "How_to_self_exam_Get_Mammo_03S.mp4",
    }
  };
  ...
  lessons.forEach(lesson => {
    lesson.addEventListener("click", () => {
      ...
      if (type === "video") {
        ...
        breadcrumb.innerText = `Diagnosis › ${title}`; // Updated breadcrumb
      } else if (type === "reading") {
        loadReading();
        readingSection.style.display = "block";
        breadcrumb.innerText = `Diagnosis › ${currentLanguage === "en" ? "Reading" : "Lectura"}`; // Updated breadcrumb
      } else if (type === "quiz") {
        loadQuiz(id);
        quizSection.style.display = "block";
        breadcrumb.innerText = `Diagnosis › ${currentLanguage === "en" ? "Quiz" : "Cuestionario"}`; // Updated breadcrumb
      }
    });
  });
